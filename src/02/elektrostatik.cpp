...
double V(double steps, double X,
         function<double(double, double, double, double)> const& f) {
  return simpson(-1, 1, steps, [X, steps, f](double x) {
    return simpson(-1, 1, steps, [X, steps, x, f](double y) {
      return simpson(-1, 1, steps,
                     [X, x, y, f](double z) { return f(x, y, z, X); });
    });
  });
}
...
int main() {
  ...
  ofstream file1;
  ofstream file2;
  file1.open("V1_out.txt");
  file2.open("V2_out.txt");
  ...
  for (int n = 11; n <= 80; n++) {
    ...
    file1 << X << "\t"
          ...
          << (1 / X) * V(steps, X, [](double x, double y, double z,
                                      double X) { return 1; }) << endl;
    file2 << X << "\t"
          << V(steps, X, [](double x, double y, double z, double X) {
               return x / sqrt((X - x) * (X - x) + y * y + z * z);
             }) << "\t"
          << 1 / (X * X) * V(steps, X, [](double x, double y, double z,
                                           double X) { return x * x; }) << endl;
  }
  file1.close();
  file2.close();

  file1.open("V1_in.txt");
  file1 << "X"
        << "\t"
        << "V" << endl;
  for (int n = 1; n <= 10; n++) {
