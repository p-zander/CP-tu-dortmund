...
plt.plot(np.linspace(-10, 10, N), psi2[:, 499], 'r-', label=r'$\vert\psi_n(t=10)\vert^2$')
plt.legend()
plt.savefig("t0_10.pdf")

plt.figure()
plt.plot(np.linspace(0, 10, t), norm, label=r'$\Sigma_n(\Delta\xi)\vert\psi_n(t)\vert^2$')
plt.ylim(0.9,1.1)
plt.legend()
plt.savefig("norm.pdf")

plt.figure()
plt.plot(np.linspace(0, 10, t), mean, label=r'$\Sigma_n(\Delta\xi)\xi_n\vert\psi_n(t)\vert^2$')
plt.legend()
